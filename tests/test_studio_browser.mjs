...
try {
  ...
  ok(await evaluate(`JSON.stringify([...document.querySelectorAll('.toolrail button')].filter(b=>b.offsetParent!==null).map(b=>b.textContent.trim()))===JSON.stringify(['Compare','Layers','Light'])`), 'the main tools are Compare, Layers, and Light');
  ok(await evaluate(`document.querySelector('#fovBtn').closest('#adv-strokes') && document.querySelector('#ctl-foveal_strength').closest('#adv-strokes') && !document.querySelector('#adv-strokes').open`), 'detail allocation is grouped in the closed Advanced fold');
  ok(await evaluate(`document.querySelector('#detailMapStatus').hidden`), 'a painting without a detail map has no extra notice');
  await evaluate(`(async()=>{window.detailProject=await (await fetch('./sample-project.json')).json();
    window.loadDetailProject=doc=>{const dt=new DataTransfer();dt.items.add(new File([JSON.stringify(doc)],'detail-test.oilpaint.json',{type:'application/json'}));const input=document.querySelector('#projFile');input.files=dt.files;input.dispatchEvent(new Event('change',{bubbles:true}));};
    window.detailFixture=structuredClone(window.detailProject);
    const map=document.createElement('canvas');map.width=13;map.height=7;const ctx=map.getContext('2d');
    ctx.fillStyle='white';ctx.fillRect(0,0,13,7);ctx.fillStyle='#333';ctx.fillRect(2,1,6,4);
    window.detailFixture.masks.foveal=map.toDataURL('image/png');window.loadDetailProject(window.detailFixture);})()`);
  await wait(`document.querySelector('#detailMapLabel').textContent==='Detail map active' && !document.querySelector('#studioBtn').disabled`);
  ok(await evaluate(`!document.querySelector('#detailMapStatus').hidden && document.querySelector('#detailMapLabel').textContent==='Detail map active'`), 'an existing detail map has a visible status');
  await click('detailMapEdit');
  ok(await evaluate(`document.querySelector('#adv-strokes').open && !document.querySelector('#panel-strokes').hidden && !document.querySelector('#fovBar').hidden`), 'the map notice opens its advanced editor');
  await click('fovDone');
  ok(await evaluate(`document.querySelector('#fovBar').hidden && document.activeElement.id==='fovBtn' && document.querySelector('#toolNone').getAttribute('aria-pressed')==='true'`), 'Done closes the map editor and restores keyboard focus');
  await evaluate(`const paused=structuredClone(window.detailFixture);paused.params.foveal_strength=0;window.loadDetailProject(paused);`);
  await wait(`document.querySelector('#detailMapLabel').textContent==='Detail map paused' && !document.querySelector('#studioBtn').disabled`);
  ok(await evaluate(`document.querySelector('#ctl-foveal_strength').value==='0' && document.querySelector('#fovStrength').value==='0'`), 'importing a paused map preserves zero influence in both controls');
  await click('detailMapClear');
  await wait(`document.querySelector('#detailMapStatus').hidden && !document.querySelector('#studioBtn').disabled`);
  await page('Input.dispatchKeyEvent', {type:'keyDown',key:'z',code:'KeyZ',modifiers:2});
  await wait(`!document.querySelector('#detailMapStatus').hidden && !document.querySelector('#studioBtn').disabled`);
  ok(await evaluate(`document.querySelector('#detailMapLabel').textContent==='Detail map paused' && document.querySelector('#ctl-foveal_strength').value==='0'`), 'undo restores a cleared map without activating it');
  ok(await evaluate(`(async()=>{
    const original=HTMLAnchorElement.prototype.click;let exported='';
    HTMLAnchorElement.prototype.click=function(){if(this.download.endsWith('.foveal.png'))exported=this.href;else original.call(this);};
    try{document.querySelector('#fovSave').click();}finally{HTMLAnchorElement.prototype.click=original;}
    if(!exported)return false;
    const a=new Uint8Array(await (await fetch(exported)).arrayBuffer());
    const b=new Uint8Array(await (await fetch(window.detailFixture.masks.foveal)).arrayBuffer());
    return a.length===b.length&&a.every((v,i)=>v===b[i]);
  })()`), 'clear and undo preserve the imported map bytes for export');
  await click('detailMapClear');
  await evaluate(`window.loadDetailProject(window.detailProject)`);
  await wait(`document.querySelector('#detailMapStatus').hidden && document.querySelector('#ctl-foveal_strength').value==='0.5' && !document.querySelector('#studioBtn').disabled`);
  await evaluate(`document.querySelector('#adv-strokes').open=false`);
  ...
  await wait(`!document.querySelector('#studioView').hidden`);
  await click('rgnBtn');
  ok(await evaluate(`document.querySelector('#studioView').hidden && !document.querySelector('#rgnBar').hidden`), 'painting tools release Studio light');
  await click('toolNone');
  await click('auto');
